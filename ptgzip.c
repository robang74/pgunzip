...
#ifndef _DEBUG
#define _DEBUG     0 //xFFFF
#endif

#ifndef _SZE_MMAP
#define _SZE_MMAP  23 //RAF: =0 to test mmap() failure, =n is 2^n min size      (ok)
#endif
#ifndef _DNT_REOR
...
#   define _SEEKER_FUNC  7
# endif
#endif //_SEEKER_FUNC

#if _SEEKER_FUNC >= 1 && _SEEKER_FUNC <= 4 // 4-bytes unaligned version
...
#elif _SEEKER_FUNC == 5  // 3-bytes SSE2 aligned version
...
fprintf(stderr, "mgk: 0x%08x\n", *(uint32_t *)inbuf);
            ...
            #elif _SEEKER_FUNC >= 5 && _SEEKER_FUNC <= 8
            /* RAF
...
static ALWAYS_INLINE
uint32_t ptgz_header_init(int infd, pgunz_t *ptbl)
{
    void *ptr = NULL;
    ...
    if (_g_read_mmap_base && _USE_MMAP) {
        ...
        if (_g_read_mmap_base) {
            ...
            len = xfull_read(infd, _g_ptgz_header,
                       PTGZ_HEADER_SIZE);
    ...
    if (_g_read_mmap_base) {
        ...
        len += xfull_read(infd,
            &_g_ptgz_header[PTGZ_HEADER_SIZE], nbytes);
...
size_t do_output_mmap(int ofd)
{
    ...
    max_size  = opt_decompress ? 0 : (WBUF_MAX_SIZE * _g_tot_chunks);
    if (max_size)
        max_size += PTGZ_HEADER_CURSIZE;
    else
        return 0;

    /* 1. Pre-allocate max size for output mmap */
    if (ftruncate(ofd, max_size) < 0) {
        perror("ftruncate");
        return 0;
    }

    if(!_SZE_MMAP || max_size < (1UL << _SZE_MMAP))
        return 0;

    /* 2. Map output file into virtual memory */
    _g_out_mmap_base = mmap(NULL, max_size,
...
// === elaboration =============================================================
    ...
    if (opt_decompress) // inflate
    {
        pgunz_t tbl, *ptbl = &tbl;
        uint32_t out_size = 0, in_size = 0, buf_size = 0;
        uint32_t next_idx = 0, *ilst = NULL;
        uint16_t nbytes = 0;
...
#if _DEBUG & 0x400 // ----------------------------------------------------------
fprintf(stderr, "PTGZ> size: %u, lsze: %lu, 1off: %lu, nchk: %d/%d, mxos: %lu\n",
    in_size, _g_ptgz_list_size, _g_first_offeset, _g_tot_chunks, tbl.nwords, max_out_size);
...
set_default_values:
            ...
            ret = _inflate_stream(infd, ofd, in_size,
                out_size, _g_ptgz_header, buf_size, !max_out_size, &tbl);
        }
        else
...
do_inflate_parall:
            ...
            ret = inflate_parallel(infd, ofd, in_size,
                out_size, NULL, buf_size, !max_out_size, &tbl, &sem);
            buf_size = 0;
        }
...
#if _DEBUG & 0x800 // ----------------------------------------------------------
fprintf(stderr, "PTGZ> size: %u, lsze: %lu, 1off: %lu, nchk: %d, mxos: %lu\n",
    in_size, _g_ptgz_list_size, _g_first_offeset, _g_tot_chunks, max_out_size);
